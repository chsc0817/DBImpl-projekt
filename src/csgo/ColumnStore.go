...
func (rl *Relation) Aggregate(aggregate AttrInfo, aggrFunc AggrFunc) Relationer {
	...
	//teile rl in groupColumns und aggrColumn
	...
	//initialisiere join columns mit signatur von groupColumn...
	...
	} //...und attrColumn als letztes
	aggrPos := len(JoinColumns)
	JoinColumns = append(JoinColumns, emptyColumn)
	JoinColumns[aggrPos].Signature = aggregate

	switch JoinColumns[aggrPos].Signature.Type {
	case INT:
	...
	//fuege jede zeile zu JoinColumns hinzu
	for i := 0; i < aggrColumnLength; i++ {
		...
		PosAggrColumns := -1
		//check if currentColumn already exists within JoinColumn (except aggrColumn)
